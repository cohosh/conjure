...
func newAMPCacheREgServer() AMPCacheRegServer {
...
func generateC2SWrapperPayload() (c2AMPCache *pb.C2SWrapper, marshaledc2AMPCache []byte) {
	generation := uint32(0)
	covert := "1.2.3.4:1234"
	...
	c2AMPCache = &pb.C2SWrapper{
		SharedSecret:        secret,
		RegistrationPayload: &c2s,
	}

	marshaledc2AMPCache, _ = proto.Marshal(c2AMPCache)
...
func TestAMPCacheGetClientAddr(t *testing.T) {
...
func TestCorrectUnidirectionalAMPCache(t *testing.T) {
	originalIP := "1.2.3.4"
	updatedIP := "4.3.2.1"
	...
	regSrc := pb.RegistrationSource_AMPCache
...
func TestCorrectBidirectionalAMPCache(t *testing.T) {
	...
	regSrc := pb.RegistrationSource_BidirectionalAMPCache
	c2sPayload.RegistrationSource = &regSrc
	c2sPayload.RegistrationAddress = net.ParseIP(originalIP).To16()
	body, _ := proto.Marshal(c2sPayload)
...
func TestBidirectionalAMPCacheClientConf(t *testing.T) {
	...
	fakeBdRegFunc := func(c2sPayload *pb.C2SWrapper, regMethod pb.RegistrationSource, clientAddr []byte) (*pb.RegistrationResponse, error) {
		if c2sPayload.RegistrationPayload.GetDecoyListGeneration() < testCCGeneration {
			t.Fatalf("outdated clientconf in c2s not updated by AMPCache before sending to be processed")
		}
