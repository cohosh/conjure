...
func Assets() *assets {
	...
	if err != nil {
		log.Warnf("error getting assets: %v", err)
	}
	...
	if assetsInstance != nil {
		...
		if dir != assetsInstance.path {
			...
			if _, err := os.Stat(dir); err != nil {
				log.Warnf("Assets path unchanged %v.\n", err)
				return assetsInstance, err
			}
			log.Warnf("Assets path changed %s->%s. (Re)initializing", assetsInstance.path, dir)
...
func getDefaultKey() []byte {
	...
	_, _ = hex.Decode(key, []byte(keyStr))
	return key
}
...
func (a *assets) readConfigs() error {
	...
	var err error
	log.Infoln("Assets: reading from folder " + a.path)
